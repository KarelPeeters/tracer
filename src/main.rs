...
fn main() {
    ...
    println!("max_compute_work_group_count: {:?}", limits.max_compute_work_group_count());
    ...
    let spheres_buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), spheres.iter().cloned()).unwrap();
    let lights_buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), lights.iter().cloned()).unwrap();
    let material_buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), materials.iter().cloned()).unwrap();
    ...
    let buffer = CpuAccessibleBuffer::from_iter(device.clone(), BufferUsage::all(), (0..size * size * 4).map(|_| 0u8))
